...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      btnMin.addEventListener("mousedown", downBtnMin);
      function downBtnMin(event) {
        var down = downRange(event, thisEl.$refs.btnMin, thisEl.$refs.sliderRange);

        console.log(down);

        document.addEventListener("mouseup", upBtnMin);
        document.addEventListener("mousemove", moveBtnMin);
      }
      function moveBtnMin() {
      ...
      function upBtnMin(){
        document.removeEventListener("mousemove",  moveBtnMin);
      }
    ...
    methods: {
      ...
      downRange(event, btn, slider, type) {
        let buttonCoords = this.getCoords(btn);
        let shiftX = event.pageX - buttonCoords.left;
